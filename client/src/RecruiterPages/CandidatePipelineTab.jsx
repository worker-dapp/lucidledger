...
const CandidatePipelineTab = () => {
  const { recruiterId, isLoading: recruiterLoading } = useRecruiter();
  ...
  }, [recruiterId, statusFilter, jobFilter]);
  ...
  const handleBulkUpdate = async (status) => {
    ...
    try {
      ...
      setMessage(error.message || "Unable to update candidates.");
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div>
          <h2 className="text-2xl font-semibold text-[#0D3B66]">Candidate Pipeline</h2>
          <p className="text-sm text-gray-500">Review applicants for jobs assigned to you and move them forward.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {["all", "pending", "accepted", "signed", "rejected", "declined"].map((status) => (
            ...
            >
              {status === "all" ? "All Candidates" : status === "declined" ? "Declined" : status.charAt(0).toUpperCase() + status.slice(1)}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
                      ...
                      <div>
                        ...
                        {job?.employer && (
                          <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                            ...
                            {job.employer.company_name || job.employer.email}
                          </p>
                        )}
                      </div>
                    </div>
        ...
        <div className="lg:w-3/5 bg-white rounded-xl border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-[#0D3B66] mb-4">Candidate Detail</h3>
          {!selectedApplication && (
            <p className="text-sm text-gray-500">Select a candidate to review details.</p>
          )}
          {selectedApplication && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-600">
                ...
                <div>
                  ...
                  <p>{selectedApplication.job?.employer?.company_name || selectedApplication.job?.employer?.email || "--"}</p>
                </div>
                <div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 text-sm text-gray-600">
                <p className="font-semibold text-[#0D3B66] mb-2">Offer Terms</p>
                <p className="mb-1">
                ...
                {selectedApplication.job?.recruiter_fee_amount && (
                  <p className="mb-1 text-green-700">
                    Recruitment fee: {parseFloat(selectedApplication.job.recruiter_fee_amount).toLocaleString()} {selectedApplication.job.recruiter_fee_currency || "USD"} (paid by employer — worker owes zero fees)
                  </p>
                )}
                <p className="text-xs text-gray-500">
                  Accepting a candidate here issues the offer; the employer handles contract deployment once it's signed.
                </p>
              </div>

              {isPendingStatus(selectedApplication.application_status) && (
                <p className="text-xs text-gray-500">
                  Use the bulk actions in the left panel to accept or reject pending candidates.
                </p>
              )}
            </div>
          )}

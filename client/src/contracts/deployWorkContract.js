...
const MEDIATOR_ADDRESS = import.meta.env.VITE_MEDIATOR_ADDRESS;
 ...
 */
export const approveUSDC = async (primaryWallet, spenderAddress, amountUSD, onStatusChange) => {
  if (!USDC_ADDRESS) {
    throw new Error("USDC address not configured (VITE_USDC_ADDRESS)");
  }
 ...
 * @param {string} [params.mediatorAddress] - Optional custom mediator (defaults to env var)
...
export const deployWorkContract = async ({
  ...
  paymentAmountUSD,
  jobId,
  mediatorAddress = MEDIATOR_ADDRESS,
  onStatusChange,
}) => {
  if (!USDC_ADDRESS) {
    throw new Error("USDC address not configured (VITE_USDC_ADDRESS)");
  ...
  if (!mediatorAddress) {
    throw new Error("Mediator address not configured (VITE_MEDIATOR_ADDRESS)");
  }
  ...
  try {
    ...
    await approveUSDC(primaryWallet, predictedAddress, paymentAmountUSD, onStatusChange);
    ...
    //
    // For MVP: We'll use the wallet client directly for deployment
    // since Dynamic Labs wallets support this.
...
export const checkConfiguration = () => {
  const required = {
    ...
    VITE_MEDIATOR_ADDRESS: MEDIATOR_ADDRESS,
  };
  ...
  return {
    ...
    config: {
      ...
      mediator: MEDIATOR_ADDRESS || "(not set)",

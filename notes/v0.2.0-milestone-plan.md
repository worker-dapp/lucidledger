...
**QR spoofing risks and mitigations:**

The design (worker generates QR, employer kiosk scans) is correct — the reverse (employer posts QR, worker scans) would allow a worker to photograph and share the code with absent coworkers. Even so, the worker-generates design has residual attack vectors:

1. **Live screen share / video call** *(most realistic)*
   Worker B (absent) opens their QR and streams their screen live to Worker A at the kiosk via FaceTime or similar. The 15–30 sec TTL doesn't protect against a live feed. Requires active coordination between two workers but is low-tech.
   *Mitigation:* on-device biometric gate (see below).

2. **Token not invalidated after first scan**
   If the backend doesn't mark the token as consumed immediately on scan, the same token could be scanned twice within the TTL window.
   *Mitigation:* hard requirement — backend must atomically mark token used on first `POST /api/presence-event`. Reject any reuse even within TTL.

3. **Offline mode token validation gap**
   The kiosk queues events in IndexedDB when offline and syncs later. But by the time connectivity returns, the 15–30 sec token has long expired — the backend can't validate it. Offline clock-ins either get silently accepted without validation (security hole) or silently rejected (breaks offline functionality).
   *This is the trickiest unresolved design problem.* Options: longer TTL for offline scenarios (weakens security), cryptographically signed tokens the kiosk can verify locally without a server round-trip, or accept that offline mode carries elevated fraud risk and log it prominently for employer review.

4. **Kiosk endpoint not device-locked**
   The kiosk POSTs directly to `/api/presence-event`. If that endpoint isn't restricted to verified kiosk sessions, anyone who obtains a valid token could POST it from anywhere without being near a kiosk.
   *Mitigation:* consider kiosk session tokens or IP/device binding, at minimum ensure endpoint requires auth.

5. **Physical handoff**
   Worker A physically takes Worker B's phone to the kiosk. Requires B to hand over their unlocked phone — a much higher bar than screen sharing, but not impossible between colluding workers.
   *Mitigation:* biometric gate (see below) makes this harder but doesn't eliminate it if B unlocks the phone and hands it over pre-authenticated.

**Potential enhancement — on-device biometric gate:**
Before generating the QR token, require the worker to authenticate via their device's native biometric prompt (Face ID / Touch ID / fingerprint). Implementation: Web Authentication API (`navigator.credentials.get()` with `userVerification: "required"`) in mobile browsers. Biometric data never leaves the device (secure enclave model) — no privacy/storage risk, no backend changes needed.

This mitigates the live screen-share attack: Worker B can't simply share their screen because generating the QR requires their biometric at that moment.

*Caveat for Lucid Ledger's user base:* many low-income workers use basic or older Android phones that lack reliable biometric hardware. This enhancement should be treated as a progressive enhancement — fail gracefully (fall back to PIN or just allow QR generation) if biometrics are unavailable, rather than blocking clock-in entirely. Do not make it a hard requirement for MVP.

**Note on employer-controlled biometric kiosks (fingerprint reader / facial recognition):**
This approach was considered and rejected for this context. While biometric kiosks are more spoof-resistant, they require workers to hand their most permanent personal data to the party the app is designed to protect them from. Coercion risk (workers unable to refuse enrollment without losing work), fingerprint degradation from manual labor (construction, agriculture), documented racial bias in facial recognition, and legal liability under biometric privacy laws (BIPA etc.) make this a poor fit for a wage protection app. On-device biometrics owned by the worker are the appropriate model here.

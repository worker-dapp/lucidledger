...
## Phase 1: Quick Bug Fixes (~1–2 hrs total)

These are one-liners or near-one-liners. Do these immediately.

### #58 — Nginx $server_name → $host
**Labels:** `bug`
**Effort:** ~10 min

In `nginx.conf`, the HTTP redirect block uses `$server_name` instead of `$host`, causing requests to `api.lucidledger.co` to redirect to `https://lucidledger.co/...`.

- **Fix:** Change `$server_name` to `$host` in the HTTP → HTTPS return 301 directive.
- **Bonus:** Add `backend` to the `depends_on` list for `nginx` in `docker-compose.nginx.yml` to prevent transient 502s on cold starts.

---

### #56 — Admin redirect bug (admin → onboarding loop)
**Labels:** `bug`, `frontend`
**Effort:** ~30 min

When an admin visits `/` while authenticated, `checkProfileAndRedirect` in `App.jsx` finds no employee profile and sends them to `/user-profile`.

- **Fix:** Check admin status before the profile lookup. Options:
  - Hit `GET /api/admin/check` (lightweight endpoint) and short-circuit if admin.
  - Thread admin status through the existing post-auth flow.
- **File:** `client/src/App.jsx` — `checkProfileAndRedirect` function.

---

## Phase 2: Security & Data Integrity (~4–6 hrs total)

Security issues should be resolved before opening up the platform further.

### #41 — Email UNIQUE constraint evaluation
**Labels:** `backend`, `security`
**Effort:** ~1 hr (mostly a decision)

The `email` column has a UNIQUE constraint but wallet address is the sole identity key. A stale record from an abandoned account could block a new user with the same email.

- **Decision options:**
  - **A) Drop the UNIQUE constraint** — safest, wallet address is the real key.
  - **B) Keep it but build admin tooling** to clear stale records (overkill for now).
  - **C) Leave as-is** — accept manual intervention if/when it happens.
- **Recommendation:** Option A. Add a migration to drop the constraint.
- **Migration:** `server/migrations/015-drop-email-unique-constraint.sql`
...
**Do now (quick items):**
- [ ] Wallet address checksum validation via `viem.getAddress()` at API boundaries
- [ ] Stricter rate limiting on admin endpoints (lower limit than global 100/15min)
- [ ] Immutable fields protection in `updateDeployedContract` (block changing `contract_address`, `employer_id`, `employee_id`)
- [ ] Dedicated `GET /api/admin/check` endpoint (also needed for #56 fix)
...
1. Add `contract_snapshot JSONB` column to `deployed_contracts` table.
   - **Migration:** `server/migrations/016-add-contract-snapshot.sql`
2. Populate snapshot at the moment of offer acceptance (or contract deployment):
   - Capture: job title, pay rate, schedule, responsibilities, benefits, start date.
   - Source from `job_postings` record at that timestamp.
3. Backend: include `contract_snapshot` in `GET /api/deployed-contracts/:id` response.
4. Frontend: render snapshot in Job Tracker's contract detail view (read-only, clearly labeled "Terms at signing").
5. Handle nulls gracefully — existing contracts won't have a snapshot; fall back to live job posting data with a note.
...
| #41 | Email UNIQUE constraint | 2 | ~1 hr |
